...
class SemanticSearchEngine:
    ...
    def process_meeting_for_search(self, meeting_id: int, transcription_text: str,
        ...
        print(f"🧩 Meeting {meeting_id} split into {len(transcription_chunks)} chunks")
    ...
    def search_meetings(self, query: str, all_embeddings: List[Dict],
                        top_k: int = 10, similarity_threshold: float = 0.3) -> List[Dict]:
        """Search across all meetings using semantic similarity"""
        print(f"🔎 Searching '{query}' in {len(all_embeddings)} chunks...")
        ...
        try:
            ...
            for item in all_embeddings:
                ...
                print(f"Similarity (meeting {item['meeting_id']} chunk {item['chunk_index']}): {similarity:.4f}")

                if similarity >= similarity_threshold:
                    results.append({
                        'meeting_id': item['meeting_id'],
                        'chunk_type': item.get('chunk_type', 'transcription'),  # fallback to safe default
            ...
            if not results:
                ...
                print(f"✅ {len(results)} chunks passed threshold.")
    ...
    def find_similar_meetings(self, meeting_id: int, meeting_embeddings: List[Dict],
        ...
        print(f"🔍 Running semantic similarity search for meeting ID: {meeting_id}")
